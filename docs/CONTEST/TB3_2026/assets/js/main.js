...
async function openModal(id) {
  ...
  document.getElementById('modal-body').innerHTML = `
    ...
    ${m.images ? `<div class="modal-section"><h3>📸 參考圖片</h3><div style="display:flex;flex-wrap:wrap;gap:0.5rem;margin-top:0.5rem;">${m.images.map(img => `<a href="${img}" target="_blank"><img src="${img}" style="width:120px;height:auto;border-radius:6px;object-fit:cover;"></a>`).join('')}</div></div>` : ''}
