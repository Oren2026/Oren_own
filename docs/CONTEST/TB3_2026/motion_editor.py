...
class MotionCommander:
    ...
    def init_ros(self):
        try:
            rospy.init_node('motion_sequence_editor', anonymous=True)
            self.pub = rospy.Publisher('/control/moving/state', MovingParam, queue_size=1)
            self.sub_complete = rospy.Subscriber('/control/moving/complete', UInt8, self.cb_complete, queue_size=1)
            self.is_connected = True
            rospy.loginfo("[MotionEditor] ROS connected")
            return True
        except Exception as e:
            self.is_connected = False
            return False

    def cb_complete(self, msg):
        if self.running:
            self.step_complete = True

    def send_moving(self, moving_type, linear=0.0, angular=0.0):
        msg = MovingParam()
        msg.moving_type = moving_type
        msg.moving_value_linear = linear
        msg.moving_value_angular = angular
        self.pub.publish(msg)
        rospy.loginfo(f"[MotionEditor] sent: type={moving_type}, linear={linear}, angular={angular}")

    def wait_complete(self, timeout=30):
        """等待 is_moving_complete"""
        self.step_complete = False
        start = rospy.Time.now()
        while not self.step_complete and not rospy.is_shutdown():
            if (rospy.Time.now() - start).to_sec() > timeout:
                rospy.logwarn("[MotionEditor] timeout waiting for move complete")
                return False
            rospy.sleep(0.05)
        return True
    ...
    def execute_sequence(self, sequence, progress_callback=None):
        ...
        for i, (mtype, val) in enumerate(sequence):
            ...
            if progress_callback:
                progress_callback(i, f"執行第 {i+1} 步...")
            if mtype == 3:
                rospy.loginfo(f"[MotionEditor] 旋轉 {val} 度")
                self.send_moving(3, angular=val)
                self.wait_complete()
            elif mtype == 4:
                rospy.loginfo(f"[MotionEditor] 前進 {val*100} 公分")
                self.send_moving(4, linear=val)
                self.wait_complete()
            elif mtype == 5:
                rospy.loginfo(f"[MotionEditor] 後退 {val*100} 公分")
                self.send_moving(5, linear=val)
                self.wait_complete()
            elif mtype == 0:
                rospy.loginfo(f"[MotionEditor] 等待 {val} 秒")
                rospy.sleep(val)
            rospy.sleep(0.2)  # 穩定一下再下一步
        self.running = False
        if progress_callback:
...
class MotionSequenceEditor:
    ...
    def _build(self):
        ...
        self.conn_label = tk.Label(left_panel, text="⚫ 未連線",
                                   fg='#888', bg='#1e1e1e', font=('Arial', 9))
        self.conn_label.pack()
        tk.Button(left_panel, text="測試 ROS 連線",
    ...
    def test_ros(self):
        def try_connect():
            ok = self.commander.init_ros()
            if ok:
                self.conn_label.config(text="⚫ 已連線 ROS", fg='#88ff88')
            else:
                self.conn_label.config(text="⚫ 連線失敗", fg='#ff8888')
                messagebox.showerror("ROS 連線失敗",
                    "無法連線 ROS，請確認：\n1. roscore 已啟動\n2. /control/moving/state topic 存在")
    ...
    def execute_sequence(self):
        ...
        if not self.commander.is_connected:
            messagebox.showerror("未連線", "請先按「測試 ROS 連線」確認 roslaunch 正常")
            return
