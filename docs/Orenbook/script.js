const PAGES = [
        ...
        let currentPage = 0; // 0-indexed: 0=first view, 1=page2, ... 21=page22
        let isReading = false;
        ...
        const TOTAL_PAGES = PAGES.length; // 22

        function updatePage() {
            // left slot = currentPage, right slot = currentPage + 1
            const leftIdx = currentPage;
            const rightIdx = currentPage + 1;

            if (leftIdx < TOTAL_PAGES) {
                leftTitle.textContent = PAGES[leftIdx].title;
                leftContent.innerHTML = PAGES[leftIdx].content;
                leftNum.textContent = leftIdx + 1;
            }

            if (rightIdx < TOTAL_PAGES) {
                rightTitle.textContent = PAGES[rightIdx].title;
                rightContent.innerHTML = PAGES[rightIdx].content;
                rightNum.textContent = rightIdx + 1;
            }

            // Chapter bar: show title of left (current) page
            const chapterName = PAGES[leftIdx].title
                .replace(/^ch\.\d+\s*/, "")
                .replace(/^序$/, "序 — 我不是ChatGPT");
            chapterBar.textContent = chapterName;

            // Buttons: can go prev if past first view, can go next if not at end
            prevBtn.style.visibility = currentPage > 0 ? "visible" : "hidden";
            nextBtn.style.visibility = rightIdx < TOTAL_PAGES - 1 ? "visible" : "hidden";

            // Indicator: left page number / total
            indicator.textContent = `${leftIdx + 1} / ${TOTAL_PAGES}`;

            // Apply slide transform
            spread.style.transform = `translateX(${-currentPage * 100}%)`;
        }

        function nextPage() {
            const rightIdx = currentPage + 1;
            if (isAnimating || rightIdx >= TOTAL_PAGES - 1) return;
