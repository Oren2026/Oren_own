...
class TokenType(Enum):
    TK_FUNC = auto(); TK_RETURN = auto(); TK_IF = auto(); TK_WHILE = auto(); TK_ID = auto(); TK_NUM = auto()
...
class Parser:
    ...
    def statement(self):
        if self.cur.type == TokenType.TK_IF:
            ...
            self.consume()                        # while
            self.consume()                        # (
            loop_start = len(self.quads)          # 條件表達式的第一個 quad 位置（★關鍵！）
            cond = self.expression()              # 產生 CMP_LT 等 quads
            self.consume(); self.consume()         # ), {
            jmp_f_idx = len(self.quads)
            self.emit("JMP_F", cond, "-", "?")    # 條件為0就跳出

            while self.cur.type != TokenType.TK_RBRACE:
                self.statement()
            self.consume()                         # }
            self.emit("JMP", "-", "-", str(loop_start))  # 回頭到條件起點（重新計算）
            self.quads[jmp_f_idx].result = str(len(self.quads))  # 回填跳出點
